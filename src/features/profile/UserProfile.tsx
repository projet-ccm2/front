      ...
      <div className="flex-1 overflow-auto bg-[#0e0e10] dark:bg-gray-50">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-8">
              ...
              <div className="flex-1 pb-4 text-center sm:text-left">
                ...
                <div className="mb-2">
                  <div className="flex items-center justify-between text-sm mb-2">
                    ...
                    <span className="text-white dark:text-gray-900">{currentXP} / {nextLevelXP} XP</span>
                  </div>
                ...
                <p className="text-gray-300 dark:text-gray-700">Just {nextLevelXP - currentXP} XP until next level!</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid sm:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
            ...
            <div className="bg-[#18181b] dark:bg-white border border-[#2d2d31] dark:border-gray-200 rounded-xl p-4 sm:p-6">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div className="text-sm text-gray-400 dark:text-gray-600">Achievements Unlocked</div>
              </div>
          ...
          <div className="bg-[#18181b] dark:bg-white border border-[#2d2d31] dark:border-gray-200 rounded-xl p-4 sm:p-8 mb-6 sm:mb-8">
            <h2 className="text-xl sm:text-2xl text-white dark:text-gray-900 mb-4 sm:mb-6">Achievement Badges</h2>
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3 sm:gap-4">
              {unlockedBadges.map((badge) => (
                <div
                  ...
                  title={badge.title}
                >
                  <div className={`text-2xl sm:text-4xl mb-1 sm:mb-2 ${!badge.unlocked && 'grayscale'}`}>
                    {badge.icon}
                  </div>
                  <div className={`text-xs text-center line-clamp-2 ${badge.unlocked ? 'text-white dark:text-gray-900' : 'text-gray-500'}`}>
                    {badge.title}
                  </div>
                </div>
              ))}
            ...
            <div className="space-y-3">
              {leaderboard.map((user) => (
                ...
                >
                  {/* Rank */}
                  <div
                    className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg flex items-center justify-center flex-shrink-0 text-sm ${user.rank === 1
                        ? 'bg-[#ffd700] text-black'
                        : user.rank === 2
                          ? 'bg-[#c0c0c0] text-black'
                          : user.rank === 3
                            ? 'bg-[#cd7f32] text-black'
                            : 'bg-[#4d4d51] dark:bg-gray-300 text-white dark:text-gray-900'
                      }`}
                  >
                    #{user.rank}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-white dark:text-gray-900 text-sm sm:text-base truncate">{user.username}</div>
                    <div className="text-xs sm:text-sm text-gray-400 dark:text-gray-600">Level {user.level}</div>
                  </div>

                  {/* XP */}
                  <div className="text-right">
                    <div className="text-[#ffd700] text-sm sm:text-base">{user.xp.toLocaleString()} XP</div>
                  </div>
                </div>
              ))}
            </div>
